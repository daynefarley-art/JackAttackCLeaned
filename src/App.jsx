...
function scoreEnd(end, cfg) {
  ...
  if (end.crossoverShot === "A") {
    a += S.crossoverShot;
    detail.push(`Crossover shot A +${S.crossoverShot}`);
  } else if (end.crossoverShot === "B") {
    b += S.crossoverShot;
    detail.push(`Crossover shot B +${S.crossoverShot}`);
  } else if (end.crossoverShot === "Both") {
    a += S.crossoverShot;
    b += S.crossoverShot;
    detail.push(`Crossover shot undecided +${S.crossoverShot} each`);
  }

  const { first, second, third } = S.rankPoints;
  if (end.first === "A") a += first;
  else if (end.first === "B") b += first;

  if (end.second === "A") a += second;
  else if (end.second === "B") b += second;

  if (end.third === "A") a += third;
  else if (end.third === "B") b += third;

  const ultimate =
    end.first && end.second && end.third && end.first === end.second && end.second === end.third;
  if (ultimate) detail.push(`Ultimate End (${first + second + third} pts sweep)`);

  const adjA = Number(end.adjA || 0);
  const adjB = Number(end.adjB || 0);
  if (adjA) {
    a += adjA;
    detail.push(`Adj A ${adjA > 0 ? "+" : ""}${adjA}`);
  }
  if (adjB) {
    b += adjB;
    detail.push(`Adj B ${adjB > 0 ? "+" : ""}${adjB}`);
  }
...
function buildCSVRows(teams, ends, cfg) {
  ...
  const rows = [header];
  const totals = ends.reduce((acc, e) => {
...
export default function App() {
  ...
  function pushHistory() {
    setHistory((h) =>
      [...h, { teams: JSON.stringify(teams), cfg: JSON.stringify(cfg), meta: JSON.stringify(meta), ends: JSON.stringify(ends) }].slice(-50)
    );
  }
  function undo() {
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-6">
        <Card>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <Label>A</Label>
              <Input value={teams.A} onChange={(e) => { pushHistory(); setTeams({ ...teams, A: e.target.value }); }} />
            </div>
            <div>
              <Label>B</Label>
              <Input value={teams.B} onChange={(e) => { pushHistory(); setTeams({ ...teams, B: e.target.value }); }} />
            </div>
          </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-3 gap-2 items-end">
            <div>
              ...
              <Input type="number" value={meta.ends} min={1} onChange={(e) => { pushHistory(); setMeta({ ...meta, ends: Number(e.target.value) }); }} />
            </div>
            <div>
              <Label>Total {teams.A}</Label>
              <div className="px-3 py-2 rounded-xl border text-sm bg-white">{totals.a}</div>
            </div>
            <div>
              <Label>Total {teams.B}</Label>
              <div className="px-3 py-2 rounded-xl border text-sm bg-white">{totals.b}</div>
            </div>
          </div>
        ...
        <Card>
          ...
          <div className="text-xs opacity-70 mt-2">
            Tip: Set Crossover Shot to 0 if your event doesn’t use that bonus.
          </div>
        </Card>
      </div>
      ...
      <Card className="mb-3">
        <div className="flex items-center justify-between">
          <div className="text-xl font-semibold">
            Scoreboard: {teams.A} {totals.a} — {totals.b} {teams.B}
          </div>
              ...
              <div className="grid md:grid-cols-6 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    value={e.aTouchers}
                    min={0}
                    onChange={(ev) => updateEnd(idx, { aTouchers: Number(ev.target.value) })}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    value={e.bTouchers}
                    min={0}
                    onChange={(ev) => updateEnd(idx, { bTouchers: Number(ev.target.value) })}
                  />
                </div>
                <div>
                  <Label>Crossover Shot</Label>
                  <select
                    ...
                    value={e.crossoverShot}
                    onChange={(ev) => updateEnd(idx, { crossoverShot: ev.target.value })}
                  >
                ...
                <div>
                  ...
                  <select
                    className="w-full px-3 py-2 rounded-xl border text-sm"
                    value={e.first}
                    onChange={(ev) => updateEnd(idx, { first: ev.target.value })}
                  >
                    <option value=""></option>
                    <option value="A">{teams.A}</option>
                    <option value="B">{teams.B}</option>
                ...
                <div>
                  ...
                  <select
                    className="w-full px-3 py-2 rounded-xl border text-sm"
                    value={e.second}
                    onChange={(ev) => updateEnd(idx, { second: ev.target.value })}
                  >
                    <option value=""></option>
                    <option value="A">{teams.A}</option>
                    <option value="B">{teams.B}</option>
                ...
                <div>
                  ...
                  <select
                    className="w-full px-3 py-2 rounded-xl border text-sm"
                    value={e.third}
                    onChange={(ev) => updateEnd(idx, { third: ev.target.value })}
                  >
                    <option value=""></option>
                    <option value="A">{teams.A}</option>
                    <option value="B">{teams.B}</option>
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={e.adjA}
                    onChange={(ev) => updateEnd(idx, { adjA: Number(ev.target.value) })}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    value={e.adjB}
                    onChange={(ev) => updateEnd(idx, { adjB: Number(ev.target.value) })}
                  />
                </div>
